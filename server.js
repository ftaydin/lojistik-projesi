...
async function connectDB() {
    try {
        ...
        db = client.db("fth_takip_sistemi");
...
app.get('/api/dashboard-stats', async (req, res) => {
    try {
        const activeTripsCount = await getCollection('trips').countDocuments({ status: 'active' });
        const availableDriversCount = await getCollection('users').countDocuments({ role: 'driver', activeTripId: null });
        const pendingTripsCount = await getCollection('trips').countDocuments({ status: 'pending' });
        const totalVehiclesCount = await getCollection('vehicles').countDocuments();
        res.json({ activeTrips: activeTripsCount, availableDrivers: availableDriversCount, pendingTrips: pendingTripsCount, totalVehicles: totalVehiclesCount });
    } catch (e) { res.status(500).json({ message: "İstatistikler alınamadı." }); }
});

// --- KULLANICI YÖNETİMİ ---
app.post('/api/register', async (req, res) => {
    const { username, password, name, role, plate } = req.body;
    if (!username || !password || !name || !role) return res.status(400).json({ success: false, message: 'Tüm zorunlu alanlar doldurulmalıdır.' });
    
    const usersCollection = getCollection('users');
    const existingUser = await usersCollection.findOne({ username });
    if (existingUser) return res.status(409).json({ success: false, message: 'Bu kullanıcı adı zaten mevcut.' });
...
app.delete('/api/users/:id', async (req, res) => {
    ...
    if (result.deletedCount === 1) res.json({ success: true, message: 'Kullanıcı başarıyla silindi.' });
    else res.status(404).json({ message: 'Kullanıcı bulunamadı.' });
});

// --- YENİ: ACİL DURUM (SOS) SİNYALİ ---
app.post('/api/sos', async (req, res) => {
    const { userId, name, plate, location } = req.body;
    // Bu bilgiyi veritabanına acil durum koleksiyonu olarak kaydedebilir,
    // e-posta veya push bildirimi gönderebilirsiniz. Şimdilik logluyoruz.
    console.log('--- ACİL DURUM SİNYALİ ALINDI ---');
    console.log(`Sürücü: ${name} (${plate})`);
    console.log(`Kullanıcı ID: ${userId}`);
    console.log(`Son Konum: ${location ? `Lat: ${location.lat}, Lng: ${location.lng}` : 'Bilinmiyor'}`);
    console.log('------------------------------------');
    // Yeni bir 'sos_alerts' koleksiyonuna bu veriyi ekleyebiliriz.
    await getCollection('sos_alerts').insertOne({ userId, name, plate, location, timestamp: new Date() });
    res.json({ success: true, message: 'SOS sinyali merkeze ulaştı.' });
});
...
app.delete('/api/vehicles/:id', async (req, res) => {
    ...
    if (result.deletedCount === 1) res.json({ success: true, message: 'Araç başarıyla silindi.' });
    else res.status(404).json({ message: 'Araç bulunamadı.' });
});

// --- DURAK YÖNETİMİ ---
app.post('/api/stops', async (req, res) => {
    const { name, location } = req.body;
    if (!name || !location || !location.lat || !location.lng) return res.status(400).json({ message: 'Durak adı ve konum zorunludur.' });
...
app.post('/api/trips', async (req, res) => {
    const { details, stops } = req.body;
    if (!details || !stops || stops.length < 2) return res.status(400).json({ message: 'Sefer detayı ve en az 2 durak zorunludur.' });
    ...
    const newTrip = {
        details, stops: orderedStops, status: 'pending', assignedDriverId: null,
        assignedDriverName: null, assignedPlate: null,
        routeColor: `#${Math.floor(Math.random()*16777215).toString(16).padStart(6, '0')}`,
        createdAt: new Date(),
    };
...
app.get('/api/driver/trip/:userId', async (req, res) => {
    ...
    if(!userObjectId) return res.json({ trip: null });
...
app.get('/api/trips/active-with-routes', async (req, res) => {
    ...
    if (activeTrips.length === 0) return res.json([]);
    const driverIds = activeTrips.map(t => t.assignedDriverId).filter(id => id);
    const activeDrivers = await getCollection('users').find({ _id: { $in: driverIds }, location: { $ne: null } }, { projection: { password: 0 } }).toArray();
    const results = activeTrips.map(trip => {
        ...
        return { ...trip, currentLocation: driver ? driver.location : null };
    });
    res.json(results);
});
